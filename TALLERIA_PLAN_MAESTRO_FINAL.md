...
### Checkpoint actual — 26 de septiembre de 2026 (fase F, cumplimiento legal RGPD/LOPDGDD: cerrada, incluido el derecho de supresión)

| Campo | Estado |
| --- | --- |
| Bloque actual | Fase F: evaluación de piloto real + cumplimiento legal (aviso legal + derecho de supresión). **Cerrada.** Fases A–E siguen cerradas (sin cambios en este bloque) |
| Estado | Continuación directa del bloque anterior de esta misma fase (aviso legal reescrito, commit `6d1168f`): ese aviso promete un derecho de supresión que la app no podía cumplir -- no existía ningún comando para borrar o anonimizar un cliente. El usuario confirmó explícitamente: anonimizar en vez de borrar físicamente, dejar rastro de auditoría, y hacerlo seguro para Supabase real. Implementado: (1) `src/lib/domain.ts` -- nuevo comando `customer_anonymize` (solo owner, como `settings`/`resource`), que sustituye nombre/teléfono/observaciones del cliente por valores neutros y borra la matrícula de sus vehículos, sin tocar solicitudes/citas/conversaciones (se conservan para el historial operativo/de garantía del taller, la decisión explícita del usuario). (2) Migración `supabase/migrations/202609260011_customer_erasure.sql` -- añade la misma rama a `execute_command` para Supabase real: el teléfono pasa a un valor fijo (`+00000000`) que sigue cumpliendo la restricción de formato de la columna `phone`, y `phone_e164` se pone a `null` (la columna de la que realmente depende tanto el índice único como la búsqueda de cliente por teléfono en altas nuevas), así el valor placeholder nunca choca entre clientes y una alta real futura con el mismo teléfono original crea un cliente nuevo en vez de resucitar el anonimizado. Cada matrícula borrada deja su propio evento `vehicle_plate_erased` en la auditoría (mismo patrón que `vehicle_corrected` ya existente), y el evento principal `customer_anonymize` quedó mapeado a `entity_type='customer'` en la lógica genérica de auditoría. **Migración preparada y con tests reales, pero NO aplicada a Supabase real todavía** -- pendiente de confirmación explícita del usuario para esa operación, igual que cualquier otra migración. (3) `src/components/editors.tsx` -- el editor de cliente ahora tiene un enlace "Anonimizar datos (derecho de supresión)" (oculto para staff y una vez ya anonimizado), que abre una confirmación explícita antes de ejecutar la acción. Probado en el navegador en modo demo: el flujo completo (confirmar → anonimizar → cliente pasa a "Cliente anonimizado" con teléfono `+00000000` → su vehículo pasa a "Sin matrícula" → reabrir el editor muestra "Datos personales ya anonimizados") funciona de punta a punta |
| Decisiones de producto confirmadas para este bloque | Anonimizar en vez de borrar físicamente; mantener solicitudes/citas/conversaciones intactas; dejar auditoría de la operación; hacerlo seguro para Supabase real (todas explícitas del usuario) |
| Alcance confirmado (bloques anteriores) | Fases A–E: **cerradas**. Aviso legal RGPD/LOPDGDD: **cerrado** (commit `6d1168f`) (ver entradas anteriores del historial) |
| Línea base | Stack, modos y funciones descritos en las secciones 2–4 |
| Rama y HEAD comprobados | `main`, HEAD `ad10f4d` (derecho de supresión) tras push; `ecae118` (checkpoint aviso legal) en el mismo historial, anterior |
| Archivos principales del bloque | `src/lib/domain.ts` (comando + lógica de demo), `src/lib/domain.test.ts` y `src/lib/supabase/database.test.ts` (tests nuevos), `src/components/editors.tsx` (UI de confirmación), `supabase/migrations/202609260011_customer_erasure.sql` (nueva, no aplicada a Supabase real) |
| Secretos del bloque | Ninguno |
| Supabase real | **Sin cambios todavía.** La migración 011 está preparada, revisada y probada (PGlite) pero no aplicada; requiere confirmación explícita del usuario antes de ejecutarla con `apply-migration.mjs --confirm` |
| Consentimiento | Sin cambios; la anonimización no toca el consentimiento histórico (`requests.consent_at`), solo los datos identificativos del cliente y la matrícula de sus vehículos |
| Automatización de BD | No usada en este bloque (ninguna consulta ni escritura contra Supabase real) |
| Commit/push histórico confirmado | `6d1168f` (aviso legal RGPD/LOPDGDD), `ecae118` (checkpoint aviso legal, fase F), `ad10f4d` (derecho de supresión: comando + migración + UI, fase F) |
| Checks de este bloque | Tests 167/167 (`pnpm run test`, incluidos 2 nuevos: uno en `domain.test.ts` y un test de integración real contra PGlite en `database.test.ts`) · Typecheck limpio (`pnpm run typecheck`) · Lint limpio (`pnpm run lint`) · Build correcto (`pnpm run build`) |
| Revisión de Codex de este bloque | No solicitada todavía. Este bloque sí toca base de datos (nueva migración) y una operación irreversible sobre datos de cliente -- candidato razonable a una revisión independiente antes de aplicar la migración 011 a Supabase real, aunque no se ha pedido explícitamente en esta sesión |
| Prueba manual real | **Hecha en modo demo.** Flujo completo probado en el navegador: confirmación, anonimización, vehículo sin matrícula, estado "ya anonimizado" al reabrir. No probado contra Supabase real porque la migración no está aplicada ahí |
| Despliegue | El push a `main` despliega el código en Vercel (comando disponible en modo Supabase real en cuanto la migración se aplique); mientras la migración 011 no esté aplicada, `customer_anonymize` fallaría contra Supabase real con "Operación no reconocida" |
| Próximo paso exacto | Pedir al usuario confirmación explícita para aplicar la migración `202609260011_customer_erasure.sql` a Supabase real antes de que el comando funcione fuera de modo demo. Fuera de eso, fase F queda cerrada; recomendaciones no bloqueantes de la evaluación de piloto siguen pendientes de priorizar (monitorización de errores, CI, confirmar plan/tier de Supabase y Vercel, dominio propio de Resend). **No se ha empezado ningún bloque nuevo más allá de este** |
