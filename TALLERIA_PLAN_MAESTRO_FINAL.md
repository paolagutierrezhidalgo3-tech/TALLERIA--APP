...
6. **Pendiente, a decidir por el usuario:** configurar `LEGAL_TAX_ID` y `LEGAL_CONTACT_EMAIL` en Vercel con los datos reales del taller piloto antes de compartir el enlace de recepción pública con clientes reales (mientras no estén, la propia página lo avisa). También quedan como recomendaciones de la evaluación de piloto (no bloqueantes, ver commit/checkpoint del 26 de septiembre): monitorización de errores en producción, CI automatizado, y confirmar el plan/tier real de Supabase y Vercel.
...
### Checkpoint actual — 26 de septiembre de 2026 (fase F, cumplimiento legal RGPD/LOPDGDD del formulario público: cerrada)

| Campo | Estado |
| --- | --- |
| Bloque actual | Fase F: evaluación de piloto real + cumplimiento legal del formulario público. **Cerrada.** Fases A–E siguen cerradas (sin cambios en este bloque) |
| Estado | El usuario pidió primero una evaluación de qué falta para un piloto real con un taller (onboarding, Supabase real, despliegue, recepción pública, gestión diaria, seguridad/permisos, notificaciones, bloqueos), separada en imprescindible/recomendable/futuro -- entregada solo como diagnóstico, sin tocar código. A partir de ahí, el usuario pidió revisar específicamente el aviso legal, la política de privacidad y los textos de protección de datos de la recepción pública pensando en RGPD/LOPDGDD para España, con placeholders para los datos propios de cada taller, sin inventar datos reales. Diagnóstico entregado (sin código) y luego implementado a petición explícita del usuario ("implementalo todo para el piloto"). Cambios: (1) `src/app/r/[slug]/aviso-legal/page.tsx` reescrito -- añade el derecho a reclamar ante la AEPD, una lista explícita de los datos que se recogen (incluida la conversación completa del chat, no solo los campos estructurados), un aviso para no incluir datos sensibles en los campos de texto libre, la edad mínima de 14 años de la LOPDGDD, Resend listado como encargado del tratamiento junto a Supabase (aclarando que Resend solo trata emails del personal, nunca datos de clientes), y una cláusula sobre Supabase que confirma que sus servidores están en la Unión Europea (Irlanda) -- verificado directamente contra el host real del pooler de Supabase (`aws-1-eu-west-1.pooler.supabase.com`) con `TALLERIA_DB_URL`, no asumido; por eso no hace falta cláusula de transferencia internacional para ese proveedor. (2) El NIF/CIF del taller y un email de contacto para protección de datos son datos reales de cada taller que esta sesión no debía inventar: se resolvieron con dos variables de entorno nuevas y opcionales, `LEGAL_TAX_ID` y `LEGAL_CONTACT_EMAIL` (sin prefijo `NEXT_PUBLIC_`, mismo patrón servidor-only que `NOTIFICATIONS_FROM_EMAIL`, documentadas en `.env.example`), con una razón concreta: cada taller piloto es su propio despliegue de Vercel, así que una variable de entorno es un sitio razonable para este dato mientras no exista un campo de Configuración para ello. Mientras no estén configuradas, la página muestra un aviso visible en rojo ("no debe compartirse este enlace con clientes reales hasta completarlo"), verificado en ambos estados (con y sin configurar) antes y después del cambio. (3) El texto del checkbox único de consentimiento en `public-reception.tsx` se ajustó de "protección de datos" a "política de privacidad" para que coincida con el nuevo título de la página -- sigue siendo una sola casilla, sin cambio funcional en el consentimiento |
| Decisiones de producto confirmadas para este bloque | Ninguna decisión de producto nueva más allá de las ya tomadas por el usuario en el pedido: usar placeholders (no datos inventados), verificar la región de Supabase antes de escribir la cláusula correspondiente, y mantener una sola casilla de consentimiento |
| Alcance confirmado (bloques anteriores) | Fases A–E: **cerradas** (ver entradas anteriores del historial) |
| Línea base | Stack, modos y funciones descritos en las secciones 2–4 |
| Rama y HEAD comprobados | `main`, HEAD `6d1168f` (aviso legal RGPD/LOPDGDD) tras push; `ba38d5e` (checkpoint fase E) en el mismo historial, anterior |
| Archivos principales del bloque | `src/app/r/[slug]/aviso-legal/page.tsx` (reescrito), `src/components/public-reception.tsx` (texto del checkbox), `.env.example` (documentación de `LEGAL_TAX_ID`/`LEGAL_CONTACT_EMAIL`). Artefactos temporales de la revisión (`.env.development.local`, `public/responsive-harness.html`) creados y eliminados en la misma sesión, no forman parte del commit |
| Secretos del bloque | Ninguno. `LEGAL_TAX_ID`/`LEGAL_CONTACT_EMAIL` no son secretos (su contenido está pensado para mostrarse en la página pública), pero siguen sin valores reales configurados en ningún entorno |
| Supabase real | Sin cambios de esquema ni de datos; se consultó (solo lectura, vía `TALLERIA_DB_URL`) el hostname del pooler para confirmar la región (`eu-west-1`), sin ejecutar ninguna escritura |
| Consentimiento | Sin cambios funcionales; solo se ajustó el texto del enlace del checkbox existente para que coincida con el nuevo título de la página del aviso legal |
| Automatización de BD | Usada solo para una consulta de solo lectura del hostname de conexión (extracción del host de `TALLERIA_DB_URL` sin imprimir credenciales), no para ninguna escritura |
| Commit/push histórico confirmado | `8d0a99a`/`2488af5`/`50f3cc8` (fase E dashboard/recepción), `2eccf72` (checkpoint dashboard), `9abde5c` (fix honeypot), `ba38d5e` (checkpoint fase E cerrada), `6d1168f` (aviso legal RGPD/LOPDGDD, fase F) |
| Checks de este bloque | Tests 165/165 (`pnpm run test`) · Typecheck limpio (`pnpm run typecheck`) · Lint limpio (`pnpm run lint`) · Build correcto (`pnpm run build`) |
| Revisión de Codex de este bloque | No solicitada (cambio de texto legal y dos variables de entorno de solo lectura en una página pública ya existente; sin tocar autorización, aislamiento entre talleres, base de datos ni el envío real del formulario) |
| Prueba manual real | **Hecha.** Página de aviso legal revisada en escritorio y móvil (320–1000px), sin desbordamientos, en los dos estados posibles: sin `LEGAL_TAX_ID`/`LEGAL_CONTACT_EMAIL` configuradas (aviso visible) y con valores de prueba configurados (aviso desaparece, NIF y email se muestran correctamente, enlace `mailto:` funcional) |
| Despliegue | El push a `main` dispara el despliegue automático de Vercel sobre la misma producción ya verificada (`https://talleria-app.vercel.app`); `LEGAL_TAX_ID`/`LEGAL_CONTACT_EMAIL` **no están configuradas ahí todavía** -- hasta que se configuren con los datos reales del taller piloto, la producción seguirá mostrando el aviso de "no compartir este enlace" |
| Próximo paso exacto | Cuando el usuario tenga los datos reales del taller piloto: configurar `LEGAL_TAX_ID` y `LEGAL_CONTACT_EMAIL` en Vercel antes de compartir el enlace de recepción pública con clientes reales. Recomendaciones no bloqueantes de la evaluación de piloto (ver historial): monitorización de errores en producción, CI automatizado, y confirmar el plan/tier real de Supabase y Vercel. **No se ha empezado ningún bloque nuevo más allá de este** |
| Después | Cuando el usuario lo priorice: el dominio propio de Resend/`NOTIFICATIONS_FROM_EMAIL` diferido en el bloque de notificaciones, u otro trabajo |
