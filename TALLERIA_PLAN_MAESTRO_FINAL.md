...
### Checkpoint actual — 26 de septiembre de 2026 (pulido visual/UX, fase E: cerrada)

| Campo | Estado |
| --- | --- |
| Bloque actual | Fase E, pulido visual/UX. **Cerrada por completo**: código y revisión visual real hechos tanto para el dashboard como para la recepción pública, con un hallazgo real encontrado y corregido en esta última |
| Estado | Sesión anterior: recepción pública (móvil, `8d0a99a`) y las 7 mejoras del dashboard (`2488af5`, `50f3cc8`); revisión visual real del dashboard hecha esa misma sesión, sin hallazgos. Esta sesión: revisión visual real de la recepción pública en móvil, pedida explícitamente por el usuario. Método: mismo enfoque que la revisión anterior del dashboard -- dev server local en modo demo (`.env.development.local` temporal, sin tocar `.env.local`) e iframe auxiliar (`public/responsive-harness.html`, temporal) para alcanzar anchos móviles/tablet, ya que la ventana de Chrome de esta máquina no baja de ~1045px. La recepción pública en modo demo resuelve el taller desde `localStorage` (slug derivado del nombre del taller demo, `taller-motor-norte`), sin necesitar Supabase real ni credenciales. **Hallazgo real encontrado**: el campo honeypot anti-bot del formulario (`<input class="sr-only" name="website">` en `public-reception.tsx`, pensado para ser invisible) no quedaba realmente oculto -- Tailwind (`@import "tailwindcss"` en la primera línea de `globals.css`) genera sus utilidades dentro de una capa de cascada CSS, y el reset propio de este archivo (`input{width:100%;padding:11px 12px;border:1px solid #dfe2eb;...}`) no está en ninguna capa, así que en CSS las reglas sin capa ganan siempre a las reglas con capa sin importar la especificidad. Eso dejaba el honeypot a ancho casi completo (con el padding y el borde del input normal), añadiendo una barra de scroll horizontal no deseada en la recepción pública en todos los anchos probados (17px de sobra en móvil, hasta 471px a 900px de ancho). Corregido añadiendo una regla `input.sr-only{...}` también sin capa (así compite en igualdad de condiciones y gana por mayor especificidad de selector) que reestablece exactamente el comportamiento de oculto-mediante-clip de Tailwind. Verificado antes y después con `scrollWidth - clientWidth` vía JavaScript en 320/390/650/900/1000px: pasó de 17-471px de desbordamiento a 0px en todos los anchos, sin tocar ningún otro estilo. Se probó también el flujo del chat (escribir nombre, enviar, avanzar de paso) tras el cambio para confirmar que seguía funcionando, y la página de aviso legal en móvil. Aparte de este hallazgo, se revisó marca/dirección/horario compactos, formulario de chat, botones y no se encontró ningún otro problema |
| Decisiones de producto confirmadas para este bloque | Ninguna decisión de producto nueva. El usuario pidió explícitamente revisar la recepción pública en móvil como continuación de la revisión del dashboard |
...
| Rama y HEAD comprobados | `main`, HEAD `9abde5c` (fix del honeypot) tras push; `2eccf72` (checkpoint del dashboard) en el mismo historial, anterior |
| Archivos principales del bloque | `src/app/globals.css` (única regla nueva, `input.sr-only`). Artefactos temporales de la revisión (`.env.development.local`, `public/responsive-harness.html`) creados y eliminados en la misma sesión, no forman parte del commit |
| Secretos del bloque | Ninguno |
| Supabase real | Sin cambios; 001–010 aplicadas y verificadas (ver sección 5) |
| Consentimiento | Sin cambios (ver sección 5); el fix no toca el checkbox de consentimiento ni el envío del formulario, solo el campo honeypot invisible |
| Automatización de BD | Sin cambios; no usada en este bloque |
| Commit/push histórico confirmado | `ed48f8a` (notificaciones), `03a8d94`/`e59c8e1` (checkpoints), `8d0a99a` (recepción pública móvil), `2488af5` y `50f3cc8` (dashboard), `2eccf72` (checkpoint dashboard), `9abde5c` (fix honeypot recepción pública) |
...
| Revisión de Codex de este bloque | No solicitada (fix de una línea de CSS, de bajo riesgo, verificado con medición antes/después; sin tocar autorización, aislamiento entre talleres, base de datos ni el envío real del formulario) |
| Prueba manual real | **Hecha.** Revisión visual real de la recepción pública en móvil con la extensión Claude in Chrome, con hallazgo encontrado y corregido (honeypot no oculto); flujo del chat probado tras el fix y funcionando. Con esto, tanto el dashboard como la recepción pública tienen su revisión visual real completa |
| Despliegue | El push a `main` dispara el despliegue automático de Vercel sobre la misma producción ya verificada (`https://talleria-app.vercel.app`) |
| Próximo paso exacto | **Fase E completamente cerrada.** Cuando el usuario lo priorice: el dominio propio de Resend/`NOTIFICATIONS_FROM_EMAIL` diferido en el bloque de notificaciones, u otro trabajo nuevo que el usuario decida. **No se ha empezado ningún bloque nuevo más allá de este** |
