...
| E. Pulido visual y UX | **En curso** | Recepción pública (móvil): marca más presente, dirección/horario compactos, chat más compacto — commiteado y desplegado; **revisión visual real en móvil queda pendiente** (sin extensión Claude in Chrome conectada en las sesiones del 25 de septiembre de 2026). Panel principal (dashboard): tipografía (Inter vía `next/font`), banner de recepción digital menos protagonista, hover en filas de la tabla de solicitudes — hecho; quedan pendientes de la propuesta acordada con el usuario: dar más énfasis a la métrica de solicitudes nuevas, revisar el efecto de cristal a medias del topbar, mostrar la agenda también entre 900–1250px en vez de ocultarla, y consolidar los grises sueltos en los tokens existentes |
...
4. **Pulido visual/UX (fase E), en curso:** recepción pública móvil (marca, contacto compacto, chat compacto) y primer bloque del dashboard (tipografía, banner de recepción, hover de tabla) commiteados y empujados el 25 de septiembre de 2026. Pendiente explícito: revisión visual real en móvil de la recepción pública (bloqueada por falta de conexión con la extensión Claude in Chrome); resto de mejoras propuestas para el dashboard (métrica de solicitudes nuevas, topbar, agenda oculta entre 900–1250px, consolidación de grises).
...
### Checkpoint actual — 25 de septiembre de 2026 (pulido visual/UX, fase E: en curso)

| Campo | Estado |
| --- | --- |
| Bloque actual | Fase E, pulido visual/UX. **En curso.** Dos partes cerradas de código (recepción pública en móvil; primer bloque del dashboard), una revisión pendiente |
| Estado | Recepción pública (móvil): marca más presente, dirección/horario visibles de forma compacta (antes ocultos por debajo de 700px), chat y formulario de revisión más compactos para reducir scroll — commiteado (`8d0a99a`) y empujado; **revisión visual real en dispositivo/navegador queda pendiente** porque la extensión Claude in Chrome no está conectada en esta sesión ni en la anterior. Dashboard: tipografía Arial/Helvetica sustituida por Inter (`next/font/google`, autoalojada, sin petición de red externa ni logic change), banner de "comparte tu enlace de recepción" reducido en protagonismo (mensaje duplicado con el widget del sidebar; ahora un único fondo `--soft-purple` plano, sin el degradado ni el collage decorativo de iconos), hover añadido a las filas de la tabla de solicitudes — commiteado (`2488af5`) y empujado. Sin cambios de funcionalidad ni de lógica en ningún caso, solo CSS y una fuente |
| Decisiones de producto confirmadas para este bloque | Ninguna decisión de producto nueva; solo estilo/UX. Confirmado con el usuario: mantener la paleta morada de marca tal cual, sin cambiarla |
...
| Rama y HEAD comprobados | `main`, HEAD `2488af5` (dashboard: tipografía/banner/hover) tras push; `8d0a99a` (recepción pública móvil) en el mismo HEAD, anterior |
| Archivos principales del bloque | `src/app/globals.css` (ambas partes), `src/app/layout.tsx` (fuente Inter vía `next/font/google`, `className` en `<html>`) |
...
| Commit/push histórico confirmado | `ed48f8a` (notificaciones), `03a8d94`/`e59c8e1` (checkpoints de notificaciones y despliegue), `8d0a99a` (recepción pública móvil: marca, contacto compacto, chat compacto), `2488af5` (dashboard: Inter, banner de recepción menos protagonista, hover de tabla) |
| Checks de este bloque | Recepción pública móvil: Tests 165/165 · Typecheck limpio · Lint limpio · Build no ejecutado en esa sesión. Dashboard (esta sesión): Tests 165/165 (`pnpm run test`) · Typecheck limpio (`pnpm run typecheck`) · Lint limpio (`pnpm run lint`) · Build correcto (`pnpm run build`, incluida la descarga/autoalojado de Inter en build) |
| Revisión de Codex de este bloque | No solicitada todavía (cambios puramente visuales, de bajo riesgo; sin tocar autorización, aislamiento entre talleres, base de datos ni recepción pública a nivel de lógica) |
| Prueba manual real | **Pendiente.** La revisión visual en móvil de la recepción pública quedó explícitamente pendiente a petición del usuario ("dejamos pendiente la revisión visual de la recepción pública en móvil"): no se ha podido abrir la extensión Claude in Chrome en ninguna de las dos sesiones de este bloque. El dashboard tampoco se ha verificado visualmente en navegador por el mismo motivo; solo se ha verificado que compila y pasa los checks automatizados |
...
| Próximo paso exacto | 1) En cuanto la extensión Claude in Chrome esté conectada: revisión visual real en móvil de la recepción pública (pendiente explícito), y revisión visual del dashboard (tipografía/banner/hover) en escritorio y móvil. 2) Continuar la fase E con el resto de mejoras propuestas para el dashboard y acordadas con el usuario pero no implementadas todavía: más énfasis visual en la métrica "solicitudes nuevas", resolver el efecto de cristal a medias del topbar (`background:#ffffffb8` sin `backdrop-filter` ni `position:sticky`), dejar de ocultar `.agenda-card` entre 900–1250px, y consolidar los grises de texto secundario sueltos en los tokens `--muted`/nuevos tokens. **No se ha empezado ningún bloque nuevo más allá de este** |
| Después | Resto de la fase E (dashboard) y, cuando el usuario lo priorice, el dominio propio de Resend/`NOTIFICATIONS_FROM_EMAIL` diferido en el bloque de notificaciones |
