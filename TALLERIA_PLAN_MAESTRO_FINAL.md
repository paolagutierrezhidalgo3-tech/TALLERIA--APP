...
### Checkpoint actual — 25 de septiembre de 2026 (notificaciones y despliegue en Vercel: cerrados)

| Campo | Estado |
| --- | --- |
| Bloque actual | Notificaciones — aviso por correo al owner/staff cuando llega una solicitud pública nueva. **Cerrado**: código validado, revisado por Codex, probado de extremo a extremo contra Supabase y Resend reales, migración aplicada, commiteado y empujado. **Despliegue en Vercel: cerrado** — producción en `https://talleria-app.vercel.app`, verificada |
| Estado | Notificaciones implementadas, con tests/typecheck/lint/build en verde, 6 rondas de revisión independiente de Codex sin hallazgos pendientes, y prueba real de extremo a extremo superada dos veces (local y en producción, ver historial). Despliegue en Vercel hecho por el usuario y verificado con comprobaciones de producción |
...
| Prueba manual real | Superada dos veces el 25 de septiembre de 2026 contra Supabase y Resend reales (detalle en el historial de abajo): local, y de nuevo en producción tras el despliegue. Reclamo atómico, cooldown y resolución de miembros validados con datos reales en ambas; entrega de email confirmada por el usuario (local, envío directo) y `SUPABASE_SECRET_KEY`/`RESEND_API_KEY` verificadas correctas en Vercel (producción) — en ambos casos la entrega a los miembros reales del taller queda pendiente de la verificación de dominio en Resend |
| Despliegue | **Vercel: hecho y verificado.** Producción en `https://talleria-app.vercel.app` (subdominio gratuito; el usuario decidió explícitamente no comprar dominio todavía). El usuario creó el proyecto, inició sesión con su cuenta real, configuró las variables de entorno de producción y las Site URL/Redirect URLs de Supabase Auth. Comprobado por Claude tras el despliegue: home/recepción pública/aviso legal responden 200; la recepción pública muestra el nombre real del taller (confirma lectura de Supabase real, no demo); `/api/notify-new-request` valida bien la entrada; prueba real de extremo a extremo contra producción confirmó `SUPABASE_SECRET_KEY` y `RESEND_API_KEY` correctas (reclamo atómico exitoso; mensaje de sandbox de Resend, no de clave inválida, visto por el usuario en los logs de Vercel); datos de prueba creados en el taller real durante esa prueba, identificados y eliminados con confirmación explícita |
| Próximo paso exacto | Diferido explícitamente por el usuario: verificar un dominio propio en Resend, configurar `NOTIFICATIONS_FROM_EMAIL`, y opcionalmente usarlo como dominio propio de Vercel. Mientras tanto, continuar con el pulido visual/UX (fase E) u otro trabajo que el usuario priorice |
