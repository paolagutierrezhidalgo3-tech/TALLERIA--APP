...
9. **Recomendaciones no bloqueantes restantes de la evaluación de piloto** (a decidir por el usuario): monitorización de errores en producción, confirmar el plan/tier real de Supabase y Vercel, y el dominio propio de Resend (ya diferido, sin fecha).
...
### Checkpoint actual — 26 de septiembre de 2026 (CI automatizado añadido; fase F sigue cerrada; LEGAL_TAX_ID/LEGAL_CONTACT_EMAIL en Vercel dejado explícitamente pendiente)

| Campo | Estado |
| --- | --- |
| Bloque actual | CI automatizado (recomendación no bloqueante de la evaluación de piloto). **Cerrado.** Fase F (cumplimiento legal + derecho de supresión, migración 011 incluida) sigue cerrada, sin cambios en este bloque |
| Estado | El usuario pidió configurar `LEGAL_TAX_ID`/`LEGAL_CONTACT_EMAIL` en Vercel; como requería datos reales del taller (NIF/CIF, email de contacto) que esta sesión no debía inventar, y no hay CLI de Vercel disponible en este entorno ni acceso al panel del usuario, se le pidieron esos datos y se le dieron los pasos exactos para configurarlo él mismo en Vercel. El usuario respondió pidiendo dejarlo pendiente por ahora y configurar en su lugar CI para el proyecto (recomendación ya registrada en la evaluación de piloto: "no hay pipeline (GitHub Actions ni similar); el despliegue es manual"). Se añadió `.github/workflows/ci.yml`: en cada push/PR a `main`, instala con pnpm (`pnpm/action-setup`, cache de pnpm vía `actions/setup-node`) y ejecuta `typecheck`, `lint`, `test` y `build`, en ese orden. No necesita ningún secreto configurado en GitHub: el build corre en modo demo (sin `NEXT_PUBLIC_DATA_MODE`/variables de Supabase, igual que cualquier checkout nuevo sin `.env.local`) y los tests de integración usan una instancia embebida de PGlite, no un proyecto Supabase real. Verificado localmente antes de commitear: se movió `.env.local` fuera del repositorio temporalmise (para simular un checkout limpio de CI) y se ejecutaron los cuatro checks completos sin él -- los 167 tests, el typecheck, el lint y el build pasaron igual; después se restauró `.env.local` sin cambios. Tras el push, se confirmó por la propia API pública de GitHub Actions (sin necesidad de `gh` CLI, que no está instalado en este entorno) que el workflow se disparó y terminó con `conclusion: success` en su primera ejecución real |
| Decisiones de producto confirmadas para este bloque | Ninguna decisión de producto; el usuario reordenó explícitamente la prioridad ("deja esto pendiente y configura CI para el proyecto") |
| Alcance confirmado (bloques anteriores) | Fases A–E y F (incluida la migración 011 en Supabase real): **cerradas** (ver entradas anteriores del historial) |
| Línea base | Stack, modos y funciones descritos en las secciones 2–4 |
| Rama y HEAD comprobados | `main`, HEAD `16c8af5` (CI) tras push; `2ab1bd2` (checkpoint migración 011 aplicada) en el mismo historial, anterior |
| Archivos principales del bloque | `.github/workflows/ci.yml` (nuevo) |
| Secretos del bloque | Ninguno; el workflow no declara ni necesita ningún secreto de GitHub |
| Supabase real | Sin cambios en este bloque |
| Consentimiento | Sin cambios |
| Automatización de BD | No usada en este bloque |
| Commit/push histórico confirmado | `6d1168f`/`ad10f4d` (fase F, código), `2ab1bd2` (checkpoint migración 011 aplicada), `16c8af5` (CI: `.github/workflows/ci.yml`) |
| Checks de este bloque | Tests 167/167, typecheck limpio, lint limpio y build correcto, verificados dos veces: localmente sin `.env.local` (simulando CI) y en la propia ejecución real de GitHub Actions tras el push (`success`) |
| Revisión de Codex de este bloque | No solicitada (archivo de configuración de CI, sin tocar código de la aplicación, base de datos, autorización ni aislamiento entre talleres) |
| Prueba manual real | No aplica (nada que revisar visualmente); la validación relevante es la propia ejecución del workflow en GitHub Actions, confirmada con éxito |
| Despliegue | No aplica un despliegue de la app; el propio CI es la novedad de este bloque. El despliegue de Vercel sigue siendo manual vía su integración con git, sin gate automático que dependa de este CI (GitHub Actions y el deploy de Vercel son independientes salvo que el usuario los conecte explícitamente más adelante) |
| Próximo paso exacto | `LEGAL_TAX_ID`/`LEGAL_CONTACT_EMAIL` en Vercel siguen **explícitamente pendientes** a petición del usuario, a la espera de los datos reales del taller piloto. Recomendaciones no bloqueantes restantes de la evaluación de piloto: monitorización de errores en producción, confirmar el plan/tier real de Supabase y Vercel, y el dominio propio de Resend (diferido, sin fecha). **No se ha empezado ningún bloque nuevo más allá de este** |
