...
### Checkpoint actual — 26 de septiembre de 2026 (fase F, cumplimiento legal RGPD/LOPDGDD: cerrada; migración 011 aplicada y verificada en Supabase real)

| Campo | Estado |
| --- | --- |
| Bloque actual | Fase F: evaluación de piloto real + cumplimiento legal (aviso legal + derecho de supresión). **Cerrada por completo, incluida la aplicación real.** Fases A–E siguen cerradas (sin cambios en este bloque) |
| Estado | Continuación directa del bloque anterior (aviso legal reescrito, `6d1168f`; comando/migración/UI del derecho de supresión, `ad10f4d`). En esta sesión el usuario confirmó explícitamente aplicar la migración `202609260011_customer_erasure.sql` a Supabase real ("sí, aplica la migración 011 a Supabase real"). Aplicada mediante `node scripts/apply-migration.mjs supabase/migrations/202609260011_customer_erasure.sql --confirm` (primero se ejecutó sin `--confirm` para confirmar la vista previa exacta del SQL antes de aplicarlo). Verificada por lectura directa del esquema real (solo lectura, vía `db-query.mjs`): el cuerpo de `execute_command` en Supabase ya contiene tanto `customer_anonymize` como `vehicle_plate_erased`, y los `grant`/`revoke` de la función quedaron exactamente igual que antes (`execute` solo para `authenticated`, además de `postgres`/`service_role` como es de esperar; nada para `anon`/`public`). El comando `customer_anonymize` ya funciona en modo Supabase real, no solo en demo |
| Decisiones de producto confirmadas para este bloque | Ninguna nueva; el usuario autorizó específicamente la aplicación de esta migración concreta a Supabase real, como exige la regla de la sección 5 para cualquier migración nueva |
| Alcance confirmado (bloques anteriores) | Fases A–E: **cerradas**. Aviso legal RGPD/LOPDGDD y derecho de supresión (código): **cerrados** (`6d1168f`, `ad10f4d`) (ver entradas anteriores del historial) |
| Línea base | Stack, modos y funciones descritos en las secciones 2–4 |
| Rama y HEAD comprobados | `main`, HEAD `bb2dd03` (checkpoint del derecho de supresión, código) al empezar esta sesión; sin cambios de código en este bloque, solo la aplicación de la migración y esta actualización del plan |
| Archivos principales del bloque | Ninguno de código (la migración ya estaba commiteada en `ad10f4d`); esta actualización de `TALLERIA_PLAN_MAESTRO_FINAL.md` |
| Secretos del bloque | Ninguno impreso ni expuesto; `TALLERIA_DB_URL` usada por los scripts existentes sin mostrarse en ningún momento |
| Supabase real | **Migración 011 aplicada y verificada.** 001–011 aplicadas en Supabase real (ver sección 5) |
| Consentimiento | Sin cambios |
| Automatización de BD | Usada para aplicar la migración (`apply-migration.mjs --confirm`, con confirmación explícita del usuario para esta operación concreta) y para verificarla después (`db-query.mjs`, solo lectura) |
| Commit/push histórico confirmado | `6d1168f` (aviso legal RGPD/LOPDGDD), `ecae118` (checkpoint aviso legal), `ad10f4d` (derecho de supresión: comando + migración + UI), `bb2dd03` (checkpoint derecho de supresión, código). Este bloque no añade código nuevo, solo la aplicación real de la migración ya commiteada y esta actualización del plan |
| Checks de este bloque | Los mismos 167/167 tests, typecheck, lint y build del bloque anterior (sin cambios de código); la migración se verificó con consultas de solo lectura contra el esquema real, no con la suite de tests (que ya la cubre vía PGlite) |
| Revisión de Codex de este bloque | No solicitada (mismo código ya revisado localmente en el bloque anterior; esta sesión solo aplicó esa migración ya escrita, con confirmación explícita del usuario) |
| Prueba manual real | Verificación de solo lectura contra Supabase real: el cuerpo de `execute_command` contiene la nueva rama y los grants de la función no cambiaron. No se ejecutó el comando `customer_anonymize` contra un cliente real de Supabase en esta sesión (solo se confirmó que la función está desplegada correctamente) |
| Despliegue | El código ya estaba desplegado en Vercel desde el push anterior; con la migración aplicada, `customer_anonymize` ya funciona también en modo Supabase real, no solo en demo |
| Próximo paso exacto | Fase F queda cerrada por completo, incluida la aplicación real. Recomendaciones no bloqueantes de la evaluación de piloto siguen pendientes de priorizar por el usuario (monitorización de errores, CI, `LEGAL_TAX_ID`/`LEGAL_CONTACT_EMAIL` en Vercel, dominio propio de Resend). **No se ha empezado ningún bloque nuevo más allá de este** |
