...
## Consentimiento en la recepción pública

Aplicar `202609230007_public_consent.sql` después de 006. Añade `requests.consent_at` (nulo para el registro manual del equipo, solo se rellena en la recepción pública) y sustituye `public_intake` por una versión con un séptimo parámetro, `p_consent`: la firma cambia, así que la migración elimina explícitamente la función de seis argumentos antes de crear la nueva, para que la versión anterior -- que nunca exigía consentimiento -- deje de ser invocable.

`/r/<slug>` enlaza a `/r/<slug>/aviso-legal`, una página de servidor con un texto orientativo (responsable, finalidad, legitimación, destinatarios, conservación, derechos) que hay que adaptar y revisar con un profesional antes de usarla con clientes reales; en modo demo no resuelve el nombre del taller y muestra el texto genérico, ya que la demo no representa datos reales. El formulario público exige marcar la casilla antes de habilitar «Enviar mi consulta»; si de todos modos llega sin marcar (llamada directa a la función, no a través de la interfaz), `public_intake` lo rechaza con una excepción explícita -- a diferencia del campo trampa o el tiempo mínimo, que fallan en silencio porque son heurísticas antibot, no un campo obligatorio real. El modo demo aplica la misma regla en `applyCommand`, así que el comportamiento es idéntico con o sin Supabase.

## Horarios estructurados

Aplicar `202609240008_business_hours.sql` después de 007. Añade `workshop_hours` (tramos semanales por día ISO, 1=lunes..7=domingo; un día sin filas está cerrado) y `workshop_hour_exceptions` (una fecha concreta que cierra el taller entero o sustituye el tramo habitual por uno propio). El campo de texto libre `workshops.hours` no cambia: sigue siendo el resumen que ven los clientes; el horario estructurado es el que ahora decide qué citas se pueden crear.

Un taller sin ninguna fila en `workshop_hours` -- todos los talleres creados antes de esta migración -- no está restringido: `is_within_business_hours` y su equivalente en `applyCommand` tratan "sin configurar" igual que el horario informativo de siempre, así que ninguna cita existente ni ningún flujo deja de funcionar. La restricción se activa taller por taller en cuanto su propietario guarda al menos un tramo desde Configuración. Una excepción con fecha concreta manda sobre el horario semanal de ese día, esté abierta con su propio tramo o cerrada del todo; una cita que cruza medianoche en la hora local del taller siempre se rechaza, porque ningún tramo cruza a otro día.

`execute_command` añade `workshop_hours` (sustituye todos los tramos del taller en una sola operación, versionado con `workshops.version`, igual que `settings`) y `workshop_hour_exception`/`workshop_hour_exception_delete` (cada excepción tiene su propia versión optimista, como clientes o recursos). Los tres son solo para el propietario. `workspace_snapshot` añade `hours` y las excepciones desde hoy en adelante (hasta 100). La comprobación de horario se ejecuta en el mismo punto de `execute_command` -- y de `applyCommand` en la demo -- que ya validaba duración y solapamiento de recursos, así que una cita rechazada por horario tampoco deja auditoría ni datos parciales.
