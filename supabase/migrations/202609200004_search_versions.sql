begin;
alter table public.requests add column version integer not null default 1 check(version>0);
alter table public.appointments add column version integer not null default 1 check(version>0);
-- Built-in Unicode normalization, no extension or external service required.
create function public.search_text(p_value text) returns text
language sql immutable parallel safe set search_path='' as $$
  select regexp_replace(normalize(lower(coalesce(p_value,'')),NFD),U&'[\0300-\036f]','','g')
$$;
revoke all on function public.search_text(text) from public,anon;
...
create or replace function public.lookup_options(p_workshop_id uuid,p_kind text,p_search text default '') returns jsonb
language plpgsql stable security invoker set search_path='' as $$
declare q text:=public.search_text(coalesce(p_search,'')); result jsonb;
begin
  ...
  if p_kind='customer' then
    select coalesce(jsonb_agg(to_jsonb(o)),'[]') into result from (
      select c.id,c.name||' · '||c.phone as label from public.customers c where workshop_id=p_workshop_id and strpos(public.search_text(c.name||' '||c.phone),q)>0 order by c.name,c.id limit 20
    ) o;
  elsif p_kind='request' then
    select coalesce(jsonb_agg(to_jsonb(o)),'[]') into result from (
      select r.id,r.version,c.name||' · '||r.reason as label from public.requests r join public.customers c on c.workshop_id=r.workshop_id and c.id=r.customer_id
      ...
      and strpos(public.search_text(c.name||' '||r.reason),q)>0 order by r.created_at desc,r.id limit 20
    ) o;
