-- Structured business hours: a weekly schedule per workshop plus dated
-- exceptions (closures/special hours), now actually enforced when creating
-- or rescheduling an appointment -- workshops.hours stays as the free-text
-- summary shown to customers, unchanged. Apply AFTER 202609230007_public_consent.sql.
-- Every existing table, policy, grant and function other than execute_command
-- and workspace_snapshot is unchanged.
begin;

-- A separate counter from workshops.version: bulk-replacing the weekly
-- schedule from the Hours section must not race with -- or invalidate --
-- unsaved edits sitting in the Configuración form, which is keyed by
-- workshops.version to reset itself whenever settings actually change.
alter table public.workshops add column hours_version integer not null default 1;

-- One row per open/close range; a day with no rows is closed. Two rows for
-- the same weekday (e.g. 09:00-14:00 and 16:00-19:00) express a lunch break
-- without any special-casing in is_within_business_hours below.
create table public.workshop_hours (
  ...
  unique(workshop_id, day_of_week, opens_at, closes_at)
);
create index workshop_hours_lookup on public.workshop_hours(workshop_id, day_of_week);
alter table public.workshop_hours enable row level security;
create policy tenant_workshop_hours on public.workshop_hours for select to authenticated using(workshop_id in (select workshop_id from public.workshop_members where user_id = (select auth.uid())));
revoke all on public.workshop_hours from public, anon, authenticated;
grant select on public.workshop_hours to authenticated;

-- A single date that overrides the regular weekly schedule: either fully
-- closed (closed=true, no times) or open with its own one-off range.
create table public.workshop_hour_exceptions (
  ...
  check ((closed and opens_at is null and closes_at is null) or (not closed and opens_at is not null and closes_at is not null and closes_at > opens_at)),
  unique(workshop_id, exception_date)
);
alter table public.workshop_hour_exceptions enable row level security;
create policy tenant_workshop_hour_exceptions on public.workshop_hour_exceptions for select to authenticated using(workshop_id in (select workshop_id from public.workshop_members where user_id = (select auth.uid())));
revoke all on public.workshop_hour_exceptions from public, anon, authenticated;
grant select on public.workshop_hour_exceptions to authenticated;

-- A workshop that has never configured a structured schedule (every
-- workshop created before this migration) has zero workshop_hours rows:
-- treated as unrestricted, exactly like the free-text hours field always
-- was, so existing appointments and workflows keep working unchanged.
-- Enforcement only turns on once the owner saves at least one range.
create function public.is_within_business_hours(p_workshop_id uuid, p_start timestamptz, p_duration_minutes integer) returns boolean
language plpgsql stable set search_path = '' as $$
declare
  tz text; p_end timestamptz; local_date date; dow smallint; exc record;
begin
  if not exists(select 1 from public.workshop_hours where workshop_id = p_workshop_id) then return true; end if;
  select timezone into tz from public.workshops where id = p_workshop_id;
  -- p_end is the actual end instant (real elapsed minutes), computed before
  -- any conversion to local time -- adding the duration to a naive local
  -- timestamp instead would silently gain or lose an hour across a DST
  -- transition. Opening/closing times are likewise anchored to the local
  -- calendar date and converted to instants via "AT TIME ZONE", so every
  -- comparison below is between real instants, never between wall-clock
  -- strings (which can repeat or skip during a DST change).
  ...
  if local_date <> (p_end at time zone tz)::date then return false; end if;
  select * into exc from public.workshop_hour_exceptions where workshop_id = p_workshop_id and exception_date = local_date;
  if found then
    if exc.closed then return false; end if;
    return p_start >= (local_date + exc.opens_at) at time zone tz and p_end <= (local_date + exc.closes_at) at time zone tz;
  end if;
  dow := extract(isodow from local_date);
  return exists(
    select 1 from public.workshop_hours
    where workshop_id = p_workshop_id and day_of_week = dow
      and p_start >= (local_date + opens_at) at time zone tz
      and p_end <= (local_date + closes_at) at time zone tz
  );
end $$;
revoke all on function public.is_within_business_hours(uuid,timestamptz,integer) from public, anon, authenticated;
