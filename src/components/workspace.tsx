...
export function Workspace() {
  ...
  const load = useCallback(async (repo: WorkshopRepository) => {
    ...
    try { setPage('dashboard'); setOffset(0); setSearch(''); setFilter('all'); queryKey.current = JSON.stringify(defaultQuery); setState(await repo.load(defaultQuery)); setScreen('app'); setError(''); }
  ...
  useEffect(() => {
    if (screen !== 'app' || !repository.current) return;
  ...
  {cancelId && <Modal title="Cancelar cita" onClose={() => setCancelId(null)}><p>La cita quedará cancelada y la solicitud volverá a estar pendiente. ¿Quieres continuar?</p><div className="detail-actions"><button className="button" onClick={() => setCancelId(null)}>Conservar cita</button><button className="button primary" disabled={busy} onClick={async () => { if (await execute({ type: 'appointment_status', id: cancelId, status: 'cancelled' })) setCancelId(null); }}>Confirmar cancelación</button></div></Modal>}
