...
describe('PostgreSQL: nuevas reglas de negocio',()=>{
 ...
 it('anonimiza un cliente (derecho de supresión) conservando su solicitud, sin permitirlo a staff, y libera el teléfono para una nueva alta real',async()=>{await asUser(userA);const cmd=intake('622333444','9999ZZZ');await execute(workshopA,cmd);const customer=(await db.query<{id:string;version:number}>('select id,version from public.customers where phone_e164=$1',['+34622333444'])).rows[0];const vehicle=(await db.query<{id:string;plate:string}>('select id,plate from public.vehicles where customer_id=$1',[customer.id])).rows[0];expect(vehicle.plate).toBe('9999ZZZ');const staff='10000000-0000-4000-8000-000000000003';await asUser(staff);await expect(execute(workshopA,{type:'customer_anonymize',id:customer.id,version:customer.version})).rejects.toThrow('propietario');await asUser(userA);await expect(execute(workshopA,{type:'customer_anonymize',id:customer.id,version:999})).rejects.toThrow('ha cambiado');await execute(workshopA,{type:'customer_anonymize',id:customer.id,version:customer.version});const anonymized=(await db.query<{name:string;phone_e164:string|null;notes:string}>('select name,phone_e164,notes from public.customers where id=$1',[customer.id])).rows[0];expect(anonymized.name).toBe('Cliente anonimizado');expect(anonymized.phone_e164).toBeNull();expect(anonymized.notes).toContain('supresión');expect((await db.query<{plate:string}>('select plate from public.vehicles where id=$1',[vehicle.id])).rows[0].plate).toBe('');expect((await db.query('select * from public.requests where customer_id=$1',[customer.id])).rows).toHaveLength(1);expect((await db.query("select * from public.audit_events where entity_id=$1 and action='customer_anonymize'",[customer.id])).rows).toHaveLength(1);expect((await db.query("select * from public.audit_events where entity_id=$1 and action='vehicle_plate_erased'",[vehicle.id])).rows).toHaveLength(1);await execute(workshopA,intake('622333444','8888YYY'));const freshCustomers=(await db.query<{id:string}>('select id from public.customers where phone_e164=$1',['+34622333444'])).rows;expect(freshCustomers).toHaveLength(1);expect(freshCustomers[0].id).not.toBe(customer.id);});
