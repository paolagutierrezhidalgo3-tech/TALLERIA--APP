...
describe('Actualizaciones obsoletas de solicitudes y citas',()=>{
 ...
 // Mirrors the boundary case fixed in supabase/migrations/202609220006_public_reception.sql's
 // generate_workshop_slug(): truncating a slug to 40 chars can land right
 // after a hyphen and reintroduce a trailing one unless it's trimmed again.
 it('genera un slug de demo sin guión final aunque el corte a 40 caracteres caiga justo en un guión',()=>{const longName='Taller '+'a'.repeat(32)+' Madrid';const s=upgradeDemo({workshop:{id:crypto.randomUUID(),name:longName,phone:'',address:'',hours:'',timezone:'Europe/Madrid',appointment_minutes:60},customers:[],vehicles:[],requests:[],conversations:[],appointments:[]});expect(s.workshop.slug?.endsWith('-')).toBe(false);expect(s.workshop.slug).toMatch(/^[a-z0-9]+(-[a-z0-9]+)*$/);});
});
